...
-   **Step 2**. Take the natural log of the likelihood $log L(\theta)$, or log-likelihood. The production now becomes the sum in log-likelihood. See @sec-calib-loglik for more details.
...
-   **Step 2**. Take the natural log of the likelihood $log L(\theta)$, product becomes sum. The `log = T` option gives the log-probability directly, which is more accurate than computing the product of probabilities and then taking its logarithm [@bolker2008].
...
```{r}
beta_val <- seq(from = 0.0001, to = 0.1, by = 0.0001)
ll_val <- sapply(log(beta_val), mll, sigma = log(1), gamma = log(2), sd_inc = log(2))
plot(beta_val, ll_val)
```

```{r}
# beta_val <- log(seq(from = 0.0001, to = 0.1, by = 0.0001))
# gamma_val <- log(seq(from = 0.3, to = 1.5, by = 0.001))
# param_val <- expand.grid(beta_val, gamma_val)
# ss_val <- with(param_val, Map(mll, Var1, Var2))
# ll_val <- sapply(log(beta_val), mll, sigma = log(1), gamma = log(2), sd_inc = log(2))
# plot(beta_val, ll_val)
```
...
# my_palette: centralized named compartment colors (see R/idm-theme.R)

ggplot(df_plot, aes(x = t, y = Inc)) +
  geom_line(color = my_palette["Inc"], linewidth = 1.2) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = my_palette["Inc"], alpha = 0.15) +
...
Use `"BFGS"` for a single-parameter fit.
