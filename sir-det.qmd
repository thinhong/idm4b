...
-   The contact rate and recovery rate are constant over time.
...
- The unit of contact rate $\beta$ is $\frac{1}{\text{time} \times \text{people}}$.
- Transmission increases as density increases.
- For aerosol contacts: COVID-19.
...
- The unit of contact rate $\beta$ is $\frac{1}{\text{time}}$.
...
Below is how we numerically solve the differential equations @eq-sir-ds, @eq-sir-di and @eq-sir-dr in R with the `odin` package.
...
sir_mod <- function(beta, gamma, S0, I0, R0, times) {
  ...
  odin_run <- odin_mod$new(beta = beta, gamma = gamma, S_init = S0, I_init = I0, R_init = R0)
