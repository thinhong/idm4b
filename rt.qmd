...
coriDemo = {
  ...
  fStyle.textContent = `
    ...
    .cori-formula .frac { display:inline-flex; flex-direction:column; text-align:center;
      line-height:1.15; }
    .cori-formula .frac .num { padding:2px 10px 5px; border-bottom:1.5px solid #64748b; }
    .cori-formula .frac .den { padding:5px 10px 2px; }
    .cori-formula .hl-num { color:#dc2626; font-weight:700; }
    .cori-formula .hl-den { color:#b45309; font-weight:700; }
    .cori-formula .rtv { font-size:24px; font-weight:800; padding:0 6px; }
    .cori-formula [data-hl] { cursor:help; border-radius:4px; padding:1px 3px;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Hover → single attribute write on the wrapper. All dimming is driven by
  // CSS rules (.cori-wrap[data-hover] .dimmable) on <g> sub-groups, so per-
  // frame work is zero. Event delegation on fMain survives innerHTML swaps.
  // ─────────────────────────────────────────────────────────────────────────
  fMain.addEventListener("pointerover", (e) => {
    const el = e.target.closest("[data-hl]");
    if (el) wrapper.setAttribute("data-hover", el.dataset.hl);
  });
...
wtSteps = {
  ...
  hint.className = "hint";
  hint.innerHTML = `
    <b>Panel 1</b> — <kbd>click</kbd> any case to set <span class="ibox">i</span>; <kbd>hover</kbd> an earlier case to preview it as <span class="jbox">j</span>. Arc labels show w while idle and switch to p<sub>ij</sub> for the hovered pair.<br>
    <b>Panel 2</b> — <kbd>click</kbd> any case to set <span class="gbox">j</span>; <kbd>hover</kbd> a later case to isolate one p<sub>ij</sub> contribution.<br>
    <b>Panel 3</b> — <kbd>click</kbd> any case to share it as <span class="gbox">j</span> with Panel 2.
  `;
  ...
  function onClickP1(cIdx) {
    selI = cIdx;
    if (hoverJ !== null &&
        (hoverJ === selI || cases[hoverJ].day >= cases[selI].day)) {
      hoverJ = null;
    }
    drawAll();
  }
  ...
  function onClickP2(cIdx) {
    selJ2 = cIdx;
    if (hoverI2 !== null &&
        (hoverI2 === selJ2 || cases[hoverI2].day <= cases[selJ2].day)) {
      hoverI2 = null;
    }
    drawAll();
  }
  ...
  const p3Net = buildNetwork(p3Svg, H3plot, "p3", (idx) => { selJ2 = idx; drawAll(); });
  ...
  function drawPanel1() {
    ...
    p1Net.nodes.forEach((n, idx) => {
      ...
      n.circle.setAttribute("stroke-width", (idx === selI || idx === hoverJ) ? 3 : 2);
    ...
    if (jSet) {
      const pos = nodePos(cases[hoverJ], H1net);
      setHdr(p1Net.hdrJ, pos, "CASE  j = " + cases[hoverJ].id, C1.jFill, hdrSide(cases[hoverJ]));
    ...
    if (iSet) {
      ...
      if (jSet) {
        ...
        pReadout.textContent = "(hover an earlier case to preview j)";
  ...
  function drawPanel2(Rjs) {
    ...
    p2Net.arcs.forEach((a, k) => {
      ...
      if (p.j === selJ2) {
        ...
        const isHover = anyHover && p.i === hoverI2;
    ...
    if (hoverI2 !== null) {
      setHdr(p2Net.hdrJ, nodePos(cases[hoverI2], H2), "CASE  i = " + cases[hoverI2].id, C2.hoverFill, hdrSide(cases[hoverI2]));
    } else {
      hideHdr(p2Net.hdrJ);
    }
  }
