  ...
  function rebuildPanels() {
    ...
    for (let i = 0; i < T; i++) {
      ...
      weightBars[i] = gIncContrib.append("rect")
        .attr("x", barX[i]).attr("width", xBW)
        .attr("fill", "#E0960A").attr("opacity", 0)
        .node();
    }
...
wtSteps = {
  ...
  style.textContent = `
    ...
    .wtsteps-wrap .panel.step3 { --c:#E0960A; --bg:#fffbeb; }
  ...
  for (let d = 1; d <= T; d++) {
    ...
    dayBoxes.push(p3Svg.append("rect")
      ...
      .attr("fill", "#fffbeb").attr("stroke", "#E0960A").attr("stroke-width", 1.5)
  ...
  function drawPanel3(Rjs) {
    ...
    p3Net.nodes.forEach((n, idx) => {
      ...
      n.circle.setAttribute("stroke", "#E0960A");
    ...
    p3Net.arcs.forEach((a, k) => {
      const p = pairs[k];
      const pij = computePij(p.i, p.j);
      a.path.setAttribute("stroke", "#E0960A");
