...
For instance, suppose $Y$ follows a Poisson distribution. We know from the exponential‐family representation, that the canonical parameter $\theta = \log(\lambda)$, where $\lambda = \mu$ because $\mathbb{E}(Y) = \lambda$ for Poisson.
