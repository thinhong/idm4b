...
export function coinSurprise(d3, ui) {
  ...
  // ── Controls ──
  const SL_p = createSlider("p  (probability of the outcome)", 0.02, 1.0, 0.01, 0.5, "#7c3aed", "purple");
  styleMathLabel(SL_p);
  ...
  for (let b = 0; b <= YMAX; b++) {
    ...
    grid.append("text")
      ...
      .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 11).attr("fill", "#94a3b8")
      .text(b);
  }
  ...
  g.append("text")
    ...
    .attr("font-family", "system-ui").attr("font-size", 12).attr("fill", "#475569")
    .text("probability  p");
  g.append("text")
  ...
  for (let k = 0; k <= 5; k++) {
    ...
    halv.append("line")
      ...
      .attr("stroke", "#cbd5e1").attr("stroke-dasharray", "2,4").attr("stroke-width", 1);
    halv.append("circle")
      .attr("cx", x(p)).attr("cy", y(s)).attr("r", 3.5)
      .attr("fill", "#fff").attr("stroke", "#7c3aed").attr("stroke-width", 2);
    halv.append("text")
      .attr("x", x(p) + (k === 0 ? -7 : 7)).attr("y", y(s) - 7)
      .attr("text-anchor", k === 0 ? "end" : "start")
      .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 10).attr("fill", "#64748b")
  ...
  const cursorDot = g.append("circle").attr("r", 6)
    .attr("fill", "#7c3aed").attr("stroke", "#fff").attr("stroke-width", 2);
  const readout = g.append("text")
    .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 13)
    .attr("font-weight", 700).attr("fill", "#7c3aed");

  // interpretation line
  const interp = document.createElement("div");
  interp.style.cssText = "font-size:13px;color:#334155;margin-top:12px;line-height:1.6;min-height:42px;";
...
export function coinEntropy(d3, ui) {
  ...
  // ── Controls ──
  const SL_p = createSlider("p  (probability of heads)", 0, 1, 0.01, 0.5, "#d97706", "amber");
  styleMathLabel(SL_p);
  const playBtn = createButton("▶  Sweep p", "go");
  ...
  card.style.cssText =
    "flex:1 1 280px;min-width:260px;border:1px solid #e2e8f0;border-radius:8px;padding:14px 16px;background:#fff;box-sizing:border-box;";
  ...
  function outcomeRow(coinChar, coinBg) {
    ...
    txt.style.cssText = "font-size:12.5px;color:#334155;line-height:1.55;";
    r.appendChild(coin); r.appendChild(txt);
    return { r, txt };
  }
  const headsRow = outcomeRow("H", "#d97706");
  const tailsRow = outcomeRow("T", "#64748b");
  ...
  barLbl.style.cssText = "font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:0.4px;margin:12px 0 5px;";
  ...
  const barH = document.createElement("div"); barH.style.cssText = "height:100%;background:#f59e0b;transition:width 0.05s;";
  const barT = document.createElement("div"); barT.style.cssText = "height:100%;background:#94a3b8;transition:width 0.05s;";
  bar.appendChild(barH); bar.appendChild(barT);
  card.appendChild(barLbl); card.appendChild(bar);

  const total = document.createElement("div");
  total.style.cssText = "font-size:14px;font-weight:700;color:#0f172a;margin-top:9px;font-family:'SF Mono',Menlo,Consolas,monospace;";
  ...
  const svg = d3.create("svg")
    ...
    .style("border", "1px solid #e2e8f0").style("border-radius", "8px").style("background", "#fff");
  ...
  for (const b of [0, 0.25, 0.5, 0.75, 1]) {
    grid.append("line").attr("x1", 0).attr("x2", innerW).attr("y1", y(b)).attr("y2", y(b)).attr("stroke", "#eef2f7");
    grid.append("text").attr("x", -8).attr("y", y(b)).attr("text-anchor", "end").attr("dominant-baseline", "middle")
      .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 10).attr("fill", "#94a3b8").text(b);
  }
  for (const p of [0, 0.25, 0.5, 0.75, 1]) {
    grid.append("text").attr("x", x(p)).attr("y", innerH + 16).attr("text-anchor", "middle")
      .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 10).attr("fill", "#94a3b8").text(p);
  }
  g.append("text").attr("x", innerW / 2).attr("y", innerH + 36).attr("text-anchor", "middle")
    .attr("font-family", "system-ui").attr("font-size", 11).attr("fill", "#475569").text("p = P(heads)");
  g.append("text").attr("transform", `translate(${-34},${innerH / 2}) rotate(-90)`).attr("text-anchor", "middle")
    .attr("font-family", "system-ui").attr("font-size", 11).attr("fill", "#475569").text("entropy  H(p)  (bits)");
  ...
  g.append("path").datum(cdata).attr("fill", "none").attr("stroke", "#d97706").attr("stroke-width", 2.5).attr("d", line);

  // max marker at p = 0.5
  g.append("line").attr("x1", x(0.5)).attr("x2", x(0.5)).attr("y1", y(1)).attr("y2", innerH)
    .attr("stroke", "#cbd5e1").attr("stroke-dasharray", "2,4");
  g.append("text").attr("x", x(0.5)).attr("y", y(1) - 6).attr("text-anchor", "middle")
    .attr("font-family", `"SF Mono",Menlo,Consolas,monospace`).attr("font-size", 10).attr("fill", "#64748b").text("max = 1 bit");

  const cur = g.append("circle").attr("r", 6).attr("fill", "#d97706").attr("stroke", "#fff").attr("stroke-width", 2);

  const cap = document.createElement("div");
  cap.style.cssText = "font-size:12px;color:#64748b;margin-top:8px;line-height:1.55;";
