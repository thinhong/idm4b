...
## Why use Gamma distribution to model waiting time

If events occur one-by-one in continuous time at a constant rate $\lambda$, (a homogeneous Poisson process), the gap between events is Exponential($\lambda$). The time until the k-th event is the sum of k independent exponential gaps
