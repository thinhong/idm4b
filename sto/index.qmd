...
The dynamics of every population has both deterministic and stochastic forces [@lande2003]. These two processes act at the same time, so real-world data often look "noisy". We can split that noise into three basic forms of stochasticity [@bolker2008], that can be classified into 2 groups:

1.  **Process noise**: the randomness within the process that generates the data. It has two forms of stochasticity [@bolker2008]:
    ...
    -   **Environmental stochasticity**: random temporal changes that affect **all individuals equally**, no matter the population size [@lande2003]. It **overtakes demographic stochasticity in large populations** [@ganyani2021]. Examples include external unpredictable events (temperature, rainfall, humidity) and individual variation (variability in contact patterns, superspreaders). These factors translate into fluctuations in the transmission parameter, often captured by overdispersion in Poisson models [@ganyani2021]. Noted that predictable changes such as seasonal changes in temperature which can be incorporate into our model in deterministic way are not counted as environmental stochasticity [@bolker2008].

2.  **Sampling/observational/measurement error**: refers to errors caused by observation or sampling methods [@dennis2006], because real data collection is never perfect. It is usually modelled by adding normally distributed variability around a mean value [@bolker2008].
...
ggplot(df[1:1000, ], aes(x = xs, y = ys, color = in_circle)) +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(-1, 1)) +
  scale_y_continuous(expand = c(0, 0), limits = c(-1, 1)) +
  scale_color_manual(values = c("#16A06B", "#E0574E")) +
  theme_idm() +
...
```{r}
#| echo: false
# Embed only raw coordinates + the inside flag; the figure derives the running
# estimate and the point counts in the browser, keeping the page lightweight.
ojs_define(data = df[c("xs", "ys", "in_circle")])
```
...
```{r}
#| echo: false
# simpi.rds stores every iteration 100..3000 for each of the 50 sample
# sizes = 145k rows, the single biggest thing bloating sto.html. The plot
# below is one convergence line across a 600px axis, so every 5th iteration
# is visually identical. Thinning here shrinks the embedded data about 5x.
df2 <- df[df$n_sims %% 5 == 0, ]
ojs_define(data2 = df2)
```
